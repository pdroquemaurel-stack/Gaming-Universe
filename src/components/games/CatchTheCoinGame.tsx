...
export function getRewardFromScore(score: number): number {
...
function getXpGainFromReward(reward: number): number {
  return reward * 2;
}

function getPercentileBeat(score: number): number {
...
function handleShare(score: number, percentile: number) {
  const text = `🎮 Je viens de faire ${score} pts sur Catch the Coin et je bats ${percentile}% des joueurs aujourd'hui sur Gaming Universe ! Tu penses me battre ? Rejoins-moi ! 💪`;
  if (typeof navigator !== 'undefined' && navigator.share) {
    navigator.share({ title: 'Gaming Universe — Catch the Coin', text }).catch(() => {});
  } else {
...
interface Props {
  onBack: () => void;
  onComplete?: (result: { score: number; coinsCollected: number; reward: number; xpGain: number }) => void;
  onUsePoints?: () => void;
  onGoShop?: () => void;
  onDownloadGame?: () => void;
  onWatchAd?: () => void;
...
export default function CatchTheCoinGame({ onBack, onComplete, onUsePoints, onWatchAd, bestScore = 0, todayPlayers = 1240 }: Props) {
  ...
  const percentileBeat = useMemo(() => getPercentileBeat(score), [score]);
  const currentXp = 650;
  const targetXp = 1000;
  ...
  useEffect(() => {
    ...
    onComplete?.({ score, coinsCollected, reward, xpGain });
  ...
  const handleCollectCoin = () => {
    ...
    window.setTimeout(() => setShowFeedback(false), 260);
  };
      ...
      <div className="relative rounded-2xl border border-orangeBrand bg-[#121212] p-4 text-center shadow-lg shadow-orangeBrand/10">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 text-left">
          <div className="rounded-xl border border-white/10 bg-black/40 p-3">
            ...
            <p className="mt-0.5 text-xl font-bold text-white">{score}</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-black/40 p-3">
            <p className="text-[10px] text-zinc-400">Meilleur score</p>
            <p className="mt-0.5 text-xl font-bold text-orangeBrand">{Math.max(bestScore, score)}</p>
          </div>
        </div>

        <div className="mt-3 rounded-xl border border-emerald-500/25 bg-emerald-500/10 p-3 text-left">
          <p className="text-sm font-semibold text-emerald-300">
            🏆 Tu bats <span className="text-white">{percentileBeat}%</span> des joueurs aujourd'hui
          </p>
          <p className="mt-0.5 text-[11px] text-zinc-400">{performanceMessage} · sur {todayPlayers.toLocaleString('fr-FR')} joueurs</p>
        </div>

        <div className="mt-3 rounded-lg border border-white/10 bg-black/30 p-3 text-left">
          <p className="text-[10px] text-zinc-400">Progression niveau : {nextXp} / {targetXp} XP</p>
          <div className="mt-2 h-2 rounded-full bg-zinc-800">
            <div className="h-full rounded-full bg-orangeBrand transition-all duration-500" style={{ width: `${progressionPercent}%` }} />
          </div>
        </div>
        ...
        <div className="mt-4 space-y-2">
          <button
            type="button"
            onClick={() => handleShare(score, percentileBeat)}
          ...
          >
            <span>📲</span> Défier un ami — Envoyer mon score
          </button>
